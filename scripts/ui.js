...
export class UI {
  ...
  renderProfile(res) {
    ...
    this.profile.innerHTML = `
        ...
        <div class="col-md-9 gap-3" id="profileButton" >
            <span class="badge fs-6 bg-primary">Açık Repolar:${res.pulic_repos}</span>
  ...
  showAlert(message, className) {
    ...
    /* uyarıyı ekrandan silme foksiyonunu showalert foksiyonu çalıştıktan 3 saniye  sonra çalıştır */
    setTimeout(() => {
      this.clearAlert();
    }, 3000);
  ...
  /* ekranı temizlemee ve bilsirim basma */
  clearProfile(e) {
    e.preventDefault();
  ...
  renderProjects(data) {
    //projeler dizisindeki her bir eleman için kart oluşturur ve ekran bas
    data.forEach((repo) => {
      this.repoArea.innerHTML += `
        ...
        <div class="col-6">
            <span class="badge bg-secondary">Yıld{ız:${repo.stargazers_count}</span>
